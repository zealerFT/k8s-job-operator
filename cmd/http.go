...
var httpCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		log.Info().Msg("k8s-job-operator api server")
...
func bootHTTPServer(cfg *config.AppConfig) {
	...
	log.Info().Msg("k8s-job-operator: boot HTTP server")
	server := http.New(
		// 控制需要展示日志的接口，屏蔽pod监控检查
	...
	<-quit
	fmt.Println("k8s-job-operator Shutting down server...")
